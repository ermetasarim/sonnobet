...
const Features = (() => {
    ...
    function updatePwaStatus() {
        ...
        const finish = () => {
            const parts = [];
            parts.push(online ? "Çevrimiçi" : "Çevrimdışı");
            if (swReady) {
...
(function FeaturesV54() {
    ...
    function onChoice(choice) {
        ...
        if (choice.score) spawnFloat(`Puan ${sign(choice.score)}`, !!choice.correct);
...
(function FeaturesV55() {
    ...
    function isUnlocked(inst) {
        const need = UNLOCK_RULES[inst] ?? 0;
        return getShiftCount() >= need;
    }

    function refreshInstitutionLocks() {
        const shifts = getShiftCount();
        const guest = window.Auth && Auth.isGuest && Auth.isGuest();
        document.querySelectorAll(".institutionBtn").forEach((btn) => {
            const inst = btn.getAttribute("data-institution");
            const need = UNLOCK_RULES[inst] ?? 0;
            let open = shifts >= need;
            if (guest) {
                open = inst === "AVM" || (window.Auth && Auth.canOpenInstitution && Auth.canOpenInstitution(inst));
            }
            ...
            if (!open) {
                ...
                if (guest && inst !== "AVM") {
                    lock.textContent = "Google ile açılır";
                } else {
                    const left = Math.max(0, need - shifts);
                    lock.textContent = left + " vardiya sonra açılır";
                }
            } else if (lock) {
                lock.remove();
            }
    ...
    function applyShiftTheme(forceShift) {
        ...
        body.classList.remove(
            "inst-AVM", "inst-Hastane", "inst-Fabrika", "inst-Site", "inst-Havalimanı", "inst-Banka"
        );
        const inst = (window.game && game.institution) || "";
        if (inst) {
    ...
    function getHintState() {
        ...
        const t = today();
        if (!s || s.date !== t) {
            const fresh = { date: t, used: 0, max: 1 };
    ...
    function useHint() {
        if (!canUseHint()) {
            if (typeof flashSaveStatus === "function") flashSaveStatus("Bugünkü ipucu kullanıldı.", true);
            return;
        }
        const area = $("choices");
        if (!area) return;
        ...
        // find correct: data or class — choices store correct on object via dataset
        let wrong = buttons.filter((b) => b.dataset.correct !== "1");
        if (wrong.length < 2) {
            // fallback: assume last applied doesn't mark — use opacity of not selected
            wrong = buttons.slice();
        }
        // hide up to 2 wrong
        let hidden = 0;
        for (const b of wrong) {
            if (b.dataset.correct === "1") continue;
        ...
        if (hidden === 0) {
            // mark by text scan from game - last event
            const ev = window.game && game.shiftDeck && game.shiftDeck[game.eventCount - 1];
            if (ev && ev.choices) {
                const correctTexts = ev.choices.filter((c) => c.correct).map((c) => c.text);
                for (const b of buttons) {
                    const t = b.querySelector(".choiceText")?.textContent || b.textContent;
                    if (!correctTexts.some((ct) => t.includes(ct.slice(0, 24)))) {
                        b.classList.add("hintFaded");
                        b.disabled = true;
                        hidden++;
                        if (hidden >= 2) break;
    ...
    function updateHintBtn() {
        ...
        if (!btn) return;
        const s = getHintState();
        const left = Math.max(0, s.max - s.used);
        btn.textContent = left ? ("💡 İpucu (" + left + ")") : "💡 İpucu yok";
        btn.disabled = left <= 0;
    ...
    function bindV55() {
            ...
            window.nextEvent = function () {
                ...
                try {
                    ...
                    if (ev2 && ev2.choices) {
                        ...
                        nodes.forEach((btn) => {
                            const text = (btn.querySelector(".choiceText") || btn).textContent || "";
                            const match = ev2.choices.find((c) => text.indexOf((c.text || "").slice(0, 20)) !== -1);
        ...
        const grid = document.querySelector(".institutionGrid");
        if (grid && !grid.__v55lock) {
            ...
            grid.addEventListener("click", (e) => {
                ...
                if (inst && !isUnlocked(inst)) {
                    ...
                    const need = UNLOCK_RULES[inst] || 0;
                    if (typeof flashSaveStatus === "function") {
                        flashSaveStatus(inst + " için " + need + " vardiya gerekli.", true);
                    } else {
                        alert(inst + " kilitli. " + need + " vardiya sonra açılır.");
                    }
                }
            }, true);
        }
...
(function FeaturesV510() {
    ...
    const CHAIN_DEFS = [
        ...
        { id: "c2", label: "Gece vardiyası bitir", check: (s) => s.nights >= 1 },
        { id: "c3", label: "Gündüz vardiyası bitir", check: (s) => s.days >= 1 },
        { id: "c4", label: "2 farklı kurum", check: (s) => Object.keys(s.inst || {}).length >= 2 },
        { id: "c5", label: "%80+ doğruluk", check: (s) => s.highAcc >= 1 },
        { id: "c6", label: "3 vardiya toplam", check: (s) => s.shifts >= 3 },
        { id: "c7", label: "3 farklı kurum", check: (s) => Object.keys(s.inst || {}).length >= 3 }
    ];
    ...
    function renderInstRepStrip() {
        ...
        if (!el) return;
        const r = getInstRep();
        const names = ["AVM", "Hastane", "Fabrika", "Site", "Havalimanı", "Banka"];
    ...
    function voiceBriefing(inst, shiftType) {
        const cfg = loadJSON(VOICE_KEY, { on: true });
    ...
    function pushArchive(entry) {
        const list = loadJSON(ARCHIVE_KEY, []);
        list.unshift(entry);
        saveJSON(ARCHIVE_KEY, list.slice(0, 30));
        renderArchive();
    }
    function renderArchive() {
        ...
        const list = loadJSON(ARCHIVE_KEY, []);
        if (!list.length) {
            el.innerHTML = "<p class='adminMuted'>Henüz arşiv yok.</p>";
            return;
        }
        el.innerHTML = list.map((r, i) =>
            `<div class="panelRow" data-i="${i}">
                <span class="panelRowText">
                    <strong>${escape(r.name)}</strong>
                    <small>${escape(r.inst)} · ${escape(r.shift)} · %${r.acc}${r.at ? " · " + escape(r.at) : ""}</small>
                </span>
                <span class="panelValue">${r.score}</span>
            </div>`
        ).join("");
    ...
    /* —— 10 A11y 2.0 —— */
    function loadA11y2() {
        return loadJSON(A11Y2_KEY, { colorblind: false, vibrate: false });
    }
