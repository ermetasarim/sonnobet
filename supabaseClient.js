...
(function () {
    ...
    async function loadProfile() {
        ...
        // Öncelik: metadata (kayıt adı) > profiles > localStorage > e-posta kökü
        const display = (metaName || dbName || stored || email.split("@")[0] || "Görevli").trim();
        profile = { id: uid, display_name: display, email };
        try {
            if (display) localStorage.setItem("son_nobet_display_name", display);
        } catch (e) {}
        // profiles satırı yoksa veya farklıysa senkronize et
        if (display && display !== dbName) {
            try {
                await client.from("profiles").upsert({ id: uid, display_name: display });
            } catch (e) {}
        }
        return profile;
    }
