...
$("startBtn").addEventListener("click", () => {
    ...
    if (window.Auth && Auth.isGuest && Auth.isGuest()) {
        if (Auth.guestCanPlay && !Auth.guestCanPlay()) {
            alert("Misafir hakkınız bitti.\nTam sürüm için Google ile giriş yapın.");
            return;
        }
    }
    ...
    game.isGuest = !!(window.Auth && Auth.isGuest && Auth.isGuest());
    ...
    grid.addEventListener("click", (e) => {
        ...
        if (window.Auth) {
            if (Auth.isGuest && Auth.isGuest()) {
                ...
                if (Auth.canOpenInstitution && !Auth.canOpenInstitution(inst)) {
                    alert("Misafir modunda sadece AVM açık.\nDiğer kurumlar için Google ile giriş yapın.");
                    return;
                }
            }
        }
...
function endShift() {
    ...
    try {
        if (window.Auth && Auth.isGuest && Auth.isGuest() && Auth.markGuestShiftDone) {
            Auth.markGuestShiftDone();
        }
    } catch (e) {}
