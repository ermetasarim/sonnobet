    ...
    listEl.innerHTML = list.map((row, i) =>
        `<div class="panelRow">
            ...
            <span class="panelRowText">
                ...
                <small>${escapeHtml(row.institution || "—")} · V${row.shift || 1}</small>
            </span>
...
function renderCareerBadge() {
    ...
    if (badgeEl) badgeEl.textContent = `V${game.careerShift || 1}`;
}
...
function autoFailTimedEvent(event) {
    ...
    if (el("radioMessage")) {
        el("radioMessage").textContent = "Süre doldu. Kararsızlık da bir karardır.";
        speakRadio(el("radioMessage").textContent);
    }
...
function renderInvestigation(ending) {
    ...
    if (!invBox) return;
    const evidence = [];
    Object.keys(game.storyFlags || {}).forEach(f => evidence.push(`Bayrak: ${f}`));
    if (game.consequences?.alarmIgnored) evidence.push("Alarm ihmali kaydı");
    if (game.consequences?.cameraBroken) evidence.push("Kamera arızası / kör nokta");
    if (cameraBonusUsed) evidence.push("Kamera kanalları incelendi");
    if ((game.handbookBonus || 0) > 0) evidence.push("Prosedür el kitabı kullanıldı");
    invBox.innerHTML = `
        <h3>Soruşturma Kurulu</h3>
        <p><strong>Sonuç:</strong> ${escapeHtml(ending.title)}</p>
        <p>${escapeHtml(ending.text)}</p>
        <p><strong>Kanıt / notlar:</strong> ${evidence.length ? escapeHtml(evidence.join(" · ")) : "Yetersiz kayıt"}</p>
        <p><strong>Ekip güveni:</strong> ${game.team?.partnerTrust ?? "-"} · <strong>Hava:</strong> ${game.weather}</p>
    `;
...
window.GameHooks = {
    ...
    onEventPresented(event) {
        ...
        if (el("eventZoneLabel")) el("eventZoneLabel").textContent = event.zone ? `Bölge: ${event.zone}` : "";
    ...
    onEndShift() {
        ...
        saveCareerMeta();
        if (!(window.Auth && Auth.isGuest && Auth.isGuest())) {
            pushLeaderboard({
...
function bindSystemsUI() {
    ...
    if (careerBtn) {
        ...
        if (meta?.playerName) {
            ...
            careerBtn.textContent = `Kariyere Devam (V${meta.careerShift || 1})`;
        }
        careerBtn.addEventListener("click", () => {
            ...
            if (el("playerName")) el("playerName").value = game.playerName;
            if (typeof SFX !== "undefined") SFX.click();
