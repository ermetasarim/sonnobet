...
const AdminPanel = (() => {
    ...
    function isAuthed() {
        // Panel erişimi = yalnızca güncel hesap admin mi?
        return isEmailAdmin();
    }
...
async function tryLogin() {
    ...
    async function renderMembersList() {
        ...
        try {
            ...
            // Sadece mevcut kolonlar (id, display_name, created_at)
            let rows = [];
            let data, error;
            ({ data, error } = await sb
                .from("profiles")
                .select("id, display_name, created_at, email, is_admin")
            ...
            if (error) {
                list.innerHTML = "";
                if (empty) {
                    empty.textContent = "Liste alınamadı (RLS?): " + (error.message || "izin yok — profiles_select_all politikası ekleyin");
            ...
            list.innerHTML = rows.map((row, i) => {
                ...
                const badge = (row.is_admin === true) ? ' <span class="adminMemberBadge">admin</span>' : "";
                ...
                return `<div class="panelRow adminMemberRow" data-uid="${uid}">
                    ...
                    <span class="panelRowText">
                        <strong class="adminMemberName">${name}</strong>${badge}
                        <small>${escapeHtml(when)}</small>
                    </span>
                    <span class="adminMemberActions">
    ...
    async function openMemberEdit(uid) {
        ...
        if ($("adminMemEmail")) $("adminMemEmail").value = row.email || "—";
        if ($("adminMemRole")) $("adminMemRole").value = (row.is_admin === true) ? "admin" : "user";
        let when = "—";
        try { if (row.created_at) when = new Date(row.created_at).toLocaleString("tr-TR"); } catch (e) {}
        if ($("adminMemMeta")) {
            $("adminMemMeta").textContent = "ID: " + row.id + " · Kayıt: " + when;
        }
    ...
    async function saveMemberEdit() {
        ...
        const role = ($("adminMemRole") && $("adminMemRole").value) || "user";
        ...
        try {
            const sb = await SNSupabase.init();
            let { error } = await sb.from("profiles").update({
                display_name: name,
                is_admin: isAdmin
            }).eq("id", uid);
            if (error && /is_admin|column|schema/i.test(String(error.message || ""))) {
                ({ error } = await sb.from("profiles").update({ display_name: name }).eq("id", uid));
                if (!error) flash("İsim kaydedildi — is_admin kolonu yok, SQL ekleyin");
            }
            if (error) {
                flash("Kayıt hatası: " + (error.message || "RLS"));
                return;
            }
            flash(isAdmin ? "Üye güncellendi (Admin)" : "Üye güncellendi (Normal)");
            await renderMembersList();
        } catch (e) {
            flash("Hata: " + (e && e.message ? e.message : e));
        }
    ...
    function bind() {
        ...
        window.onSNAuthChange = function (logged, profile) {
            ...
            if (!logged || !(profile && profile.is_admin)) clearAdminSessionFlag();
            updateAdminButtonVisibility();
        };
