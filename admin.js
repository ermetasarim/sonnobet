    ...
    async function renderMembersList() {
        ...
        try {
            ...
            list.innerHTML = rows.map((row, i) => {
                ...
                return `<button type="button" class="panelRow adminMemberRow" data-uid="${escapeHtml(row.id)}">
                    <span class="panelRank">#${i + 1}</span>
                    <span class="panelRowText">
                        <strong>${name}</strong>${badge}
                        <small>${escapeHtml(when)}</small>
                    </span>
                    <span class="panelValue adminMemberEditBtn">Düzenle</span>
                </button>`;
            }).join("");
            list.querySelectorAll(".adminMemberRow").forEach((btn) => {
                btn.addEventListener("click", () => openMemberEdit(btn.getAttribute("data-uid")));
            });
        } catch (e) {
            list.innerHTML = "";
